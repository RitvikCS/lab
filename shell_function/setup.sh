...
print_header() {
  ...
  local total_padding=$((header_width - content_length - 2))

  local left_padding=$((total_padding / 2))
  local right_padding=$((total_padding - left_padding))

  if [ $left_padding -lt 1 ]; then
    left_padding=1
    right_padding=1
  fi
  ...
  printf "${BLUE}║${NC}%*s${BOLD}${WHITE}%s${NC}%*s${BLUE}║${NC}\n" $left_padding "" "${icon} ${title} ${icon}" $right_padding ""
  echo -e "${BLUE}╚$(printf '═%.0s' $(seq 1 $((header_width - 2))))╝${NC}"
}
...
configure_eks() {
  ...
  run_command "aws eks update-kubeconfig --region \$(aws configure get region) --name $cluster_name"
...
show_help() {
  echo -e "${BOLD}Enhanced Instance Setup Script${NC}"
  echo ""
...
main() {
  ...
  run_command "pip3 install kubernetes openshift pyyaml jinja2 netaddr"
  ...
  if newgrp docker <<<'docker --version' >/dev/null 2>&1; then
    echo -e "${GREEN}  [OK] Docker working without sudo (via newgrp)${NC}"
  else
    echo -e "${YELLOW}  [INFO] Docker installed, but may need logout/login for sudo-less access${NC}"
    echo -e "${CYAN}  [TIP] Run: newgrp docker  (to activate in current session)${NC}"
  fi
